...
func createHeader(ctx *sdk.Context) string {
	...
	fields := []field{
		...
		{key: "config", value: ctx.ConfigPath},
		{key: "schema", value: ctx.SchemaPath},
		{key: "queries", value: ctx.QueriesPath},
	}
...
func (gen *Generator) Generate(ctx *sdk.Context, opts *Options) (err error) {
	...
	defer func() {
		if err := f.Close(); err != nil {
			logger.Warn("error closing file", "filepath", fn, "error", err)
		}
	}()
	...
	for _, k := range schema.Keyspaces {
		...
		for _, t := range k.Tables {
			err := func() error {
				...
				defer func() {
					if err := f.Close(); err != nil {
						gen.logger.Error("error closing file", "filepath", fn, "error", err)
					}
				}()
...
func (gg *Generator) generateKeyspaceStructs(ctx *sdk.Context, req *generateKeyspaceStructsRequest) (*generateKeyspaceStructsResponse, error) {
	...
	buf := &bytes.Buffer{}
	_, _ = buf.WriteString(createHeader(ctx))
